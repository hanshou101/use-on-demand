...
export class CDirective_Helper {
  ...
  public static initImgLazyLoad(
    ...
    cfg: LazyLoad_Cfg = ImgLazyLoad_Directive.__defaultCfg,
  ) {
    // 替代图
    const placeholderImg = cfg.placeholderImg || ImgLazyLoad_Directive.__defaultCfg.pxTransparentImg;
    // 指令ID
    const directiveId    = cfg.directiveId || ImgLazyLoad_Directive.__defaultCfg.directiveId;
    // 绑定
    const _directive     = ImgLazyLoad_Directive.getDirective(placeholderImg);
