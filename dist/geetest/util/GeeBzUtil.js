...
export class GeeBzUtil {
     ...
     * 内部SDK调用，唤起一次极验。
     */
    static __innerSdkInit(data, toListen_GeeCtrlCbs) {
    ...
    static beginGee(preInfoApi, onFingerSuc, // WARN 此处，不采用Promise的原因，是Promise天然适合【立即执行Verify】的逻辑；可能造成误解。
        ...
        return new Promise(async (resolve, reject /*WARN 用不到*/) => {
            ...
            switch (stableE) {
                ...
                case GeeStableE.SuccessGeetest_true_1: {
                    ...
                    //
                    const geeCtrl = await GeeBzUtil.__innerSdkInit(data, {
                        onReadySuc: (_geeCtrl) => {
                        ...
                        onFingerSuc: (_geeCtrl) => {
                            const { geetest_challenge, geetest_validate, geetest_seccode } = _geeCtrl.getValidate();
                            onFingerSuc({
                                geetest_challenge,
                                geetest_validate,
                                geetest_seccode,
