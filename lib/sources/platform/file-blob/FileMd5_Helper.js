...
var xX_FileMd5_Helper = /** @class */ (function () {
    ...
    xX_FileMd5_Helper.downloadAndCreateHash = function (remoteUrl) {
        ...
        return fetch(remoteUrl)
            ...
            .then(function (blob) {
            return _this.calculateMD5____CryptoJS_ArrayBuffer_Correct(blob);
        })
            .then(function (hash) {
     ...
     */
    xX_FileMd5_Helper.calculateMD5____CryptoJS_ArrayBuffer_Correct = function (blob) {
        return new Promise(function (resolve) {
            var reader = new FileReader();
            reader.readAsArrayBuffer(blob); // TIP 以【ArrayBuffer】读取
            reader.onloadend = function () {
                get_CryptoJS().then(function (CryptoJS) {
                    var wordArray = CryptoJS.lib.WordArray.create(reader.result), hash = CryptoJS.MD5(wordArray).toString();
                    resolve(hash);
                });
            };
        });
     ...
     * 						2.无法处理【网络传输】的【噪音】问题。
     */
    xX_FileMd5_Helper.calculateMD5_CustomMD5_String_Error = function (blob) {
        return new Promise(function (resolve) {
            var reader = new FileReader();
            reader.readAsText(blob); // TIP 以【String】读取
            reader.onloadend = function () {
                var res = reader.result;
                get_Md5_Helper().then(function (_a) {
                    var xX_MD5_Helper = _a.xX_MD5_Helper;
                    var hash = xX_MD5_Helper.hash(res); // WARN 将返回的结果，做Hash化。
