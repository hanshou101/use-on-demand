 ...
 */
export function xX_getQueryField(key, likeQuery // 如果外部没有传入，则取默认值
) {
 ...
 */
export function xX_fixbug_locationReplace(url) {
...
export function xX_checkHtmlVersion() {
    // 获取query参数
    var Request = xX_getFullQueryObj();
 ...
 *        1.方案A。
 */
export function xX_smoothJump(id) {
    console.log('开始jump');
 ...
 *        1.方案B。
 */
export function xX_scrollToSmooth_methodB(element, to, duration) {
    if (duration <= 0) {
        return;
    }
 ...
 *                第三方登录，弹出新网页
 */
export function xX_openWindow(url, // 新窗口网页地址
title, // 标题
w, h) {
