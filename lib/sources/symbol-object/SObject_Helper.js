...
var xX_SObject_Helper = /** @class */ (function () {
     ...
     */
    xX_SObject_Helper.flatJson_toKeyChain = function (json, prevKey, resultMap) {
        var _this = this;
        if (prevKey === void 0) { prevKey = ''; }
        if (resultMap === void 0) { resultMap = {}; }
        Object.keys(json).forEach(function (newKey) {
            // 拼接成【a.b】。（a可能已是多节。）
            var thisKey = "" + (prevKey ? prevKey + "." : '') + newKey;
            // 如果仍是【对象】或【数组】。
            [
                xX_Zepto_TypeDetectE.object,
                xX_Zepto_TypeDetectE.array,
            ].includes(_this.typeDetect_judgeType_inZeptoLib(json[newKey]))
                ? _this.flatJson_toKeyChain(json[newKey], thisKey, resultMap)
