...
var MiniCity_three95 = /** @class */ (function () {
    function MiniCity_three95() {
     ...
     *            2.【检验手机平台：适配】
     */
    MiniCity_three95.prototype.startCity = function () {
        //
    ...
    // TIP 检测手机平台
    MiniCity_three95.prototype.checkUserAgent = function () {
     ...
     */
    // 构建辅助网格系统（模拟地砖效果）
    MiniCity_three95.prototype.buildAuxSystem = function () {
        // 绘制网格线
    ...
    MiniCity_three95.prototype.buildLightSystem = function () {
    ...
    // 构造建筑物
    MiniCity_three95.prototype.buildBuilding = function () {
    ...
    // TIP 大平板（作为地基板块）
    MiniCity_three95.prototype.Building_add_Plane = function () {
        // 绘制大平板（作为地基板块）
        var planeGeometry = new THREE.BoxBufferGeometry(320, 6, 320);
        var plane = MyThree_Util.makeMesh('lambert', planeGeometry, 0x6F5F6A);
        plane.position.y = -3;
        this.scene.add(plane);
    };
    // TIP 外层的白色大理石围墙。
    MiniCity_three95.prototype.Building_add_Fense = function () {
        var fenseCoords = [
            [-130, -130],
            [-130, 130],
            [130, 130],
        ...
        var fenseShape = MyThree_Util.makeShape(fenseCoords);
        var fenseGeometry = MyThree_Util.makeExtrudeGeometry(fenseShape, 3);
        var fense = MyThree_Util.makeMesh('lambert', fenseGeometry, 0xE5CABF);
        this.scene.add(fense);
    };
    // TIP 一圈绿色的矮小灌木丛
    MiniCity_three95.prototype.Building_add_Green = function () {
        var greenCoords = [
            [-120, -120],
            [-120, 120],
            [120, 120],
        ...
        var greenShape = MyThree_Util.makeShape(greenCoords);
        var greenGeometry = MyThree_Util.makeExtrudeGeometry(greenShape, 3);
        var green = MyThree_Util.makeMesh('lambert', greenGeometry, 0xC0C06A);
        this.scene.add(green);
    };
    // TIP 添加长在【矮小灌木丛】之上的大树
    MiniCity_three95.prototype.Building_add_Trees = function () {
        var _this = this;
    ...
    // TIP 主体大医院建筑。
    MiniCity_three95.prototype.Building_add_Hospital = function () {
        // 生成。放置。
        var hospital = BuildingHelper.create_Hospital();
        hospital.position.z = -20;
        this.scene.add(hospital);
    };
    MiniCity_three95.prototype.Building_add_Lamps = function () {
        var _this = this;
        // 路灯位置。（x坐标、y坐标、旋转弧度）
        var lampsPosition = [
            [-12.5, 12.5, 1.25],
    ...
    // TIP 建造道路
    MiniCity_three95.prototype.buildRoad = function () {
        ...
        var roadBorderOuterShape = MyThree_Util.makeShape(roadBorderOuterCoords, roadBorderOuterHoleCoords);
        var roadBorderOuterGeometry = MyThree_Util.makeExtrudeGeometry(roadBorderOuterShape, 0.1);
        var roadBorderOuter = MyThree_Util.makeMesh('phong', roadBorderOuterGeometry, roadColor);
        ...
        var roadBorderInnerShape = MyThree_Util.makeShape(roadBorderInnerCoords);
        var roadBorderInnerGeometry = MyThree_Util.makeExtrudeGeometry(roadBorderInnerShape, 0.1);
        var roadBorderInner = MyThree_Util.makeMesh('phong', roadBorderInnerGeometry, roadColor);
        ...
        var roadLines = MyThree_Util.makeMesh('phong', roadLinesGeometry, roadColor);
        road.add(roadLines);
        // TIP end，线条
        this.scene.add(road);
    };
    // 建造静止汽车
    MiniCity_three95.prototype.buildStaticCars = function () {
        var _this = this;
        // 汽车位置。（x坐标、y坐标、旋转弧度）
        var carsPosition = [
            [-84, 82, 1.5],
    ...
    // 建造移动汽车
    MiniCity_three95.prototype.buildMovingCars = function () {
        var _this = this;
        // 汽车位置。（x坐标、y坐标、旋转弧度）
        var carsPosition = [
            [-130, 145, 0],
    ...
    // TIP 循环执行代码
    MiniCity_three95.prototype.loop = function () {
        // stats.update()
    ...
    // TIP 监听浏览器窗口大小变化
    MiniCity_three95.prototype.onWindowResize = function () {
        var _this = this;
    ...
    return MiniCity_three95;
}());
export { MiniCity_three95 };
...
var BuildingHelper = /** @class */ (function () {
    ...
    BuildingHelper.createItem_Lamp = function () {
        ...
        var pillar = MyThree_Util.makeMesh('phong', pillarGeomertry, 0xEBD1C2);
        lamp.add(pillar);
        // 连接处（路灯头和路灯杆子的连接）
        var connectGeometry = new THREE.CubeGeometry(10, 1, 1);
        var connect = MyThree_Util.makeMesh('phong', connectGeometry, 0x2C0E0E);
        connect.position.set(3, 30, 0);
        lamp.add(connect);
        // 路灯头
        var lightGeometry = new THREE.CubeGeometry(6, 2, 4);
        var light = MyThree_Util.makeMesh('phong', lightGeometry, 0xEBD1C2);
    ...
    BuildingHelper.createItem_Tree = function (_x, _y, _z) {
        ...
        var treeTrunk = MyThree_Util.makeMesh('lambert', treeTrunkGeometry, 0x8A613A);
        treeTrunk.position.y = 8;
        tree.add(treeTrunk);
        var treeLeafsGeometry = new THREE.BoxBufferGeometry(8, 8, 8);
        var treeLeafs = MyThree_Util.makeMesh('lambert', treeLeafsGeometry, 0x9C9E5D);
...
var HospitalHelper = /** @class */ (function () {
    ...
    HospitalHelper.create_Hospital_WhiteBase = function () {
        ...
        var base = MyThree_Util.makeMesh('lambert', baseGeometry, 0xFFFFFF);
        base.position.y = 1;
        return base;
    };
    HospitalHelper.create_Hospital_FrontMain = function () {
        var frontMainShape = MyThree_Util.makeShape(this.frontMainCoords);
        // 前栋大楼。
        var frontMainGeometry = MyThree_Util.makeExtrudeGeometry(frontMainShape, 100);
    ...
    HospitalHelper.create_Hospital_FrontRooftop = function () {
        var frontTopShape = MyThree_Util.makeShape(this.frontMainCoords);
        // 前栋大楼的天台石质板块。
        var frontTopGeometry = MyThree_Util.makeExtrudeGeometry(frontTopShape, 5);
        var frontTop = MyThree_Util.makeMesh('lambert', frontTopGeometry, 0xB1A7AF);
    ...
    HospitalHelper.create_Hospital_FrontRooftop_Shelf = function () {
        ...
        var frontRoofShelf = MyThree_Util.makeMesh('phong', frontRoofShelfGeometry, 0xFFFFFF);
    ...
    HospitalHelper.create_Hospital_FrontSecondFloor_GardenPlane = function () {
        ...
        var fronPlat = MyThree_Util.makeMesh('lambert', frontPlatGeometry, 0x0792A5);
    ...
    HospitalHelper.create_Hospital_FrontSecondFloor_Langan_BlueWall = function () {
        ...
        var frontPlatVertical = MyThree_Util.makeMesh('phong', frontPlatVerticalGeometry, 0x0792A5);
    ...
    HospitalHelper.create_Hospital_FrontSecondFloor_Langan_WhiteFushou = function () {
        ...
        var frontPlatVerticalWhite = MyThree_Util.makeMesh('phong', frontPlatVerticalWhiteGeometry, 0xFFFFFF);
    ...
    HospitalHelper.create_Hospital_Pillar_UnderFrontSecondFloor = function () {
        ...
        var frontPlatPillar = MyThree_Util.makeMesh('lambert', frontPlatPillarGeometry, 0xFFFFFF);
    ...
    HospitalHelper.create_Hospital_FrontMain_VerticalEdge_White = function () {
        ...
        var frontBorderVerticleMesh = MyThree_Util.makeMesh('phong', frontBorderVerticleGeometry, 0xFFFFFF);
    ...
    HospitalHelper.create_Hospital_FrontRooftop_EdgeWall = function () {
        ...
        var frontRoofShape = MyThree_Util.makeShape(frontRoofCoords, frontRoofHolePath);
        var frontRoofGeometry = MyThree_Util.makeExtrudeGeometry(frontRoofShape, 8);
        var frontRoof = MyThree_Util.makeMesh('phong', frontRoofGeometry, 0xFFFFFF);
    ...
    HospitalHelper.create_Hospital_BackMain_YellowWrapShell = function () {
        ...
        var backMainShape = MyThree_Util.makeShape(backMainCoords, backMainHolePath);
        var backMainGeometry = MyThree_Util.makeExtrudeGeometry(backMainShape, 90);
        var backMain = MyThree_Util.makeMesh('lambert', backMainGeometry, 0xF2E21B);
        return backMain;
    };
    HospitalHelper.create_Hospital_BackMain = function () {
        ...
        var backMiddleShape = MyThree_Util.makeShape(backMiddleCoords, backMiddleHolePath);
        var backMiddkeGeometry = MyThree_Util.makeExtrudeGeometry(backMiddleShape, 165);
        var backMiddle = MyThree_Util.makeMesh('lambert', backMiddkeGeometry, 0xFFFFFF);
    ...
    HospitalHelper.inner_create_Hospital_Window = function () {
        ...
        var glass = MyThree_Util.makeMesh('phong', glassGeometry, 0x6A5E74);
        windowObj.add(glass);
        var windowBorderGeometry = new THREE.BoxBufferGeometry(22, 2, 2);
        var windowBorder = MyThree_Util.makeMesh('phong', windowBorderGeometry, 0xFFFFFF);
