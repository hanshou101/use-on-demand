...
var Cfg = {
    ...
    localPort: 12345,
    // TIP 静态文件地址
    staticPath: express.static(path.join(__dirname, 
...
function coreCorsHandle(req, res) {
    if (Cfg.needCompleteCors) {
        console.log('采用完全CORS');
        var _origin = req.get('origin');
        res.header('Access-Control-Allow-Origin', ((!_origin || ['null', 'undefined'].includes(_origin)) // 如果不存在
            // ? 'http://www.baidu.com'
            ? '*'
            : _origin));
 ...
 * 辅助工具
 */
var xX_ProxyServer_Helper = /** @class */ (function () {
    function xX_ProxyServer_Helper() {
    }
    // TIP——————————————————————————————————————————公共方法——————————————————————————————————————————
    /**
     * 开始代理
     */
    xX_ProxyServer_Helper.start = function (serverUrl) {
        var _this = this;
        ...
        app.options('*', function (req, res) {
            console.warn("\n\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u3010\u9884\u3011\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\n");
            _this.__cors_options(req, res); // 处理跨域
            res.send(200);
        });
        // 处理其它正式请求。
        app.use('/', function (req, res) {
            console.warn('\n——————————————————————————————【正式请求】——————————————————————————————\n');
            var url = Cfg.serverUrl + req.url;
            console.log('请求的URL：', req.url, '请求的Content-Type：', req.headers);
            _this.__cors_otherReqs(req, res); // 处理跨域
     ...
     * 错误的方式
     */
    xX_ProxyServer_Helper.__wrong_start = function () {
        var proxyMiddleWare = require('http-proxy-middleware');
        var cfg = function () {
            var proxyPath = 'http://47.75.112.14:8080'; //目标后端服务地址(公司同事电脑地址)
            return {
                proxyPath: proxyPath,
    ...
    // TIP——————————————————————————————————————————私有方法——————————————————————————————————————————
    xX_ProxyServer_Helper.__cors_options = function (req, res) {
        // 日志
        logParams(req);
        // 处理部分关键逻辑
     ...
     *
     */
    xX_ProxyServer_Helper.__cors_otherReqs = function (req, res) {
        // 日志
        logParams(req);
    ...
    return xX_ProxyServer_Helper;
}());
export { xX_ProxyServer_Helper };
//# sourceMappingURL=ProxyServer_Helper.js.map