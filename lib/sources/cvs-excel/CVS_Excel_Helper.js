...
export class CVS_Excel_Helper {
     ...
     */
    static exportCsv(inputData, filename = 'export.csv') {
     ...
     * 从【】，触发【下载Excel】操作
     */
    static downloadExcel(url, filename = 'export.xls') {
        // 创建隐藏的可下载链接
     ...
     * 从Excel文件中，获取某些行列。
     */
    static readArr_fromExcel(excel_file, // 传入的Excel文件
    sheet_field, // 筛选特定字段
    inputDom) {
        return new Promise((resolve, reject) => {
            getXLSX().then((XLSX) => {
                // 声明回调
                const fileOnLoadCb = function (event) {
    ...
    export_json_to_excel({ header, _data, filename = 'excel-list', autoWidth = true } = {}) {
