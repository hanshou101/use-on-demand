...
export class xX_My_RemCompatible_Util {
   ...
   *
   */
  public static calc_remCompatible_YanNan(
    designWidth: number,          // 设计图尺寸
    adjustRatio: number = 2,      // 为方便计算的整数比例
  ) {
    // 自动计算html字体
    let htmlFSize!: number;
    ...
    // 一些转换方法
    const regSizeMethods = (__rootSize: number) => {
      // TIP 新增加的，修复方法。
