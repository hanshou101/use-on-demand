...
let wx_access_token_json: WxAccessToken_Bean | null = null;
let wx_access_token_lastUpdateTime                  = new Date().getTime();           // TIP 服务器启动时，则自动记录当前时间。
let expireTime                                      = (7200 - 500) * 1000;       // TIP 暂定为7200秒。（再减去500秒，因为考虑到延时）


// 获取Token
function wx_getToken_apiHandler (req: e.Request, res: e.Response, next: e.NextFunction) {
