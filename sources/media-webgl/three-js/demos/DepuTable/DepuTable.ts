...
class TableHelper {
   ...
   */
  public static roundedRect (x: number, y: number, width: number, height: number, radius: number): THREE.Shape {
...
class PokerHelper {
  ...
  public static async createOne_FlowerPoker (w = 14.5, h = 20): Promise<THREE.Mesh> {
    ...
    const finalTexture: () => THREE.Mesh = () => {
      ...
      materialArray.push(new THREE.MeshLambertMaterial({
        map: pokerTexture,
        // side: THREE.FrontSide,   // 这条属性，在这里没有任何作用
      }));
      materialArray.push(new THREE.MeshBasicMaterial({color: 0xFFFFFF}));
  ...
  // 随机放置扑克位置。
  public static randomPokerPos (poker: THREE.Mesh) {
    poker.position.set(
      (Math.random() - 0.5) * 200,
      (Math.random() + 0.2) * 130 + (-5),
      (Math.random() - 0.5) * 130,
    );
    poker.rotation.set(
