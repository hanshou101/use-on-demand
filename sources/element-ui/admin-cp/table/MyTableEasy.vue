...
export default class MyTableEasy extends Father_BaseVue {    // 混入在此处，进行添加。
  ...
  public render(h: (tagName: string, datas?: object, children?: string | any[]) => any): any {
    ...
    let $slots          = (this as any).$scopedSlots;
    let children: any[] = this.resortTableCols.map((item: any) => {
      let type                    = item.type;
      let label                   = item.label;
      let hasSlots                = item.name in $slots;
      // element-ui table-column 的props
      let elementTableColumnAttrs = item.elementTableColumnAttrs || {};
      let options: any            = {
        /**
      ...
      switch (type) {
        case 'text':
          if (!hasSlots) {
            ...
            let scopedEl: (data: any) => any = $slots[item.name];
            if (scopedEl) {
              scopedSlots = {
                default: (props: any) => {
                  return scopedEl({
        ...
        case 'enumTag':
          scopedSlots = {
            default: (props: any) => {
              ...
              let content = this.getEnum_fromField(item.selectOption, this.anyToString(props.row[item.prop]));

              let type = this.getEnumColor(item, props.row);
      ...
      let el = h('el-table-column', options);
      return el;
    });
