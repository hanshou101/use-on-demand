...
// export default class HelloWorld extends BaseVueClass {
  export default class Tinymce extends xX_Father_BaseVue {    // 混入在此处，进行添加。
    ...
    initTinymce() {
      ...
      console.log(`记录initTinymce1`, this);
      console.log('dom元素', document.getElementById(this.tinymceId));
      ...
      window.tinymce.init({
        ...
        init_instance_callback      : (editor: MyMceEditor) => {
          console.log(`记录init_instance_callback`, this);
          if (self.value) {
            editor.setContent(self.value);
          }
          self.hasInit = true;
          editor.on('NodeChange Change KeyUp SetContent', () => {
            console.log(`记录 NodeChange Change KeyUp SetContent `, this);
            this.hasChange = true;
            this.$emit('input', editor.getContent());
          });
        },
        setup(editor: MyMceEditor) {
          console.log(`记录setup`, this);
          console.log('dom元素', document.getElementById(self.tinymceId));

          editor.on('FullscreenStateChanged', (e: any) => {
            console.log(`记录FullscreenStateChanged`, this);
            self.fullscreen = e.state;
          });

          editor.on('init', (e) => {
            console.log(`记录init`, e, this);
          });
        },
        // 整合七牛上传
    ...
    destroyTinymce() {
      console.log(`记录destroyTinymce`, this);
      ...
      // 删除指定
      // window.tinymce.remove('#your_id');

    }

    setContent(value: any) {
      console.log(`记录setContent`, this);
      window.tinymce.get(this.tinymceId).setContent(value);
    }

    getContent() {
      console.log(`记录getContent`, this);
      window.tinymce.get(this.tinymceId).getContent();
    }

    imageSuccessCBK(arr: any[]) {
      console.log(`记录imageSuccessCBK`, this);
      const self = this;
      arr.forEach(v => {
    ...
    public mounted(): void {
      console.log('记录mounted');
      // FIXME 此处，不知为何；必须要【$nextTick】才能二次加载！！！
      // FIXME 此处，不知为何；必须要【$nextTick】才能二次加载！！！
      // FIXME 此处，不知为何；必须要【$nextTick】才能二次加载！！！
      // FIXME 此处，不知为何；必须要【$nextTick】才能二次加载！！！
      // FIXME 此处，不知为何；必须要【$nextTick】才能二次加载！！！
      // FIXME 此处，不知为何；必须要【$nextTick】才能二次加载！！！
      this.$nextTick(() => {
        console.log('记录$nextTick');
        this.initTinymce();
        // (window as any).testFn = this.initTinymce;
      });
    }
